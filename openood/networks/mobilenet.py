...
class MobileNet(nn.Module):
    ...
    def get_layer_list(self):
        ...
        for layer in self.features.children():
            layers.append(layer)
        layers.append(self.adaptive_avg)
        layers.append(nn.Flatten())
        layers.append(self.classifier)
        return layers

    def forward(self, x, return_feature=False, return_feature_list=False):
        if return_feature_list or return_feature:
            feature_list = []
            for layer in self.get_layer_list()[:-2]:
                x = layer(x)
        ...
        if return_feature:
            return x, feature_list[-1].view(x.shape[0], -1)
        return x

    def forward_threshold(self, x, threshold):
        x = self.features(x)
        features = self.adaptive_avg(x)
