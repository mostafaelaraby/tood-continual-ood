...
def get_normalization(dataset_name):
    try:
        ...
        std = mean
        print(
            "Could not find normalization for dataset {} giving error "
...
class NoiseProtoCompute:
    ...
    def update_noise_proto(self, feats):
        ...
        self.noise_proto = (
            feats_sum + self._count_features[0] * self.noise_proto
    ...
    def get_uniform_noise(self, n, inp_channel, img_size):
        ...
        for _ in range(n_iters):
            noise_imgs = torch.rand((bsz, inp_channel, img_size, img_size)).to(
                self.device
            )
            # smoothes the neighbor of the uniform image
            noise_imgs = torchvision.transforms.GaussianBlur(
                kernel_size=(5, 7), sigma=(0.2, 5)
            )(noise_imgs)

            logits, features = self.network(
                noise_imgs, return_feature_list=True
            )
            ...
            top_samples = torch.topk(
                energyconf, actual_bsz, dim=0
            ).indices.view(-1)
            penultimate_feature = features[-1][top_samples]
            self.update_noise_proto(
                penultimate_feature.contiguous().view(actual_bsz, -1)
            )
            pbar.update(bsz)
    ...
    def get_val_ood(self, n):
        ...
        for ood_data in self.ood_loader_dict["val"]:
            ...
            self.update_noise_proto(features_ood[-1].contiguous().view(bsz, -1))
            pbar.update(bsz)
            count_features += bsz
            if count_features >= n:
                break
    ...
    def get_synthetic_ood(self, n):
        ...
        for id_data in self.id_loader_dict["train"]:
            ...
            bsz = real_id.shape[0]
            logits, perturbed_features = self.network(
                real_id, return_feature_list=True
            )
            ...
            for i in range(bsz):
                ...
                print(
                    "MSP score for generated OOD {:.4f}".format(
                        tmp_logits.softmax(dim=1).max(1).values.item()
                    )
                )
                print(
                    "MSP score for ID {:.4f}".format(
                        logits[i]
                ...
                print(
                    "Logit margin between top two classes {:.4f}".format(
                        logit_margin
                    )
                )
                print("****************************************************")
...
class BaseModel(nn.Module):
    ...
    def __init__(self, net: nn.Module):
        ...
        self.flatten = (
            False
            if hasattr(self.model, "no_flatten") and self.model.no_flatten
            else True
        )
    ...
    def intermediate_forward(
        self, x, layer_start=0, layer_end=-1, return_logits=False
    ):
...
def get_mixed_feats(
    ...
    mapped_top_k_targets = (
        torch.tensor([indx_map[t.item()] for t in top_k_targets.flatten()])
        .reshape(top_k_targets.shape)
        .to(top_k_targets.device)
    )

    all_mixed_input = feats_weighted.unsqueeze(1) + proto_weighted

    expanded_indices = mapped_top_k_targets.view(
        -1, k, *([1] * len(feat_shape))
    ).expand(-1, -1, *feat_shape)

    mixed_inputs = torch.gather(all_mixed_input, 1, expanded_indices).view(
        -1, *feat_shape
    )
...
class GrOODPostprocessor(BasePostprocessor):
    ...
    def __init__(self, config):
        ...
        self.distance_loss = (
            True
            if hasattr(self.args, "distance_loss") and self.args.distance_loss
            else False
        )
        ...
        self.use_val = (
            True
            if hasattr(self.args, "use_val") and self.args.use_val
            else False
        )
        ...
        self.use_gaussian_noise = (
            ...
            and self.args.use_gaussian_noise
            else False
        )
        # flag to include validation OOD in the computation of noise centroid
        self.use_val_ood = (
            True
            if hasattr(self.args, "use_val_ood") and self.args.use_val_ood
            else False
        )
        # flag to include Synthetic OOD in the computation of noise centroid
        self.use_synthetic = (
            True
            if hasattr(self.args, "use_synthetic") and self.args.use_synthetic
            else False
        )
        self.synthetic_name = (
            self.args.synthetic_name
            if hasattr(self.args, "synthetic_name")
            else "synthetic"
        )
        # flag to use mean of prototypes as our prototype computation
        self.use_proto_mean = (
            True
            if hasattr(self.args, "use_proto_mean") and self.args.use_proto_mean
            else False
        )
        # flag to use cosine similarity instead of euclidean distance in Faiss
        self.use_cosine = (
            True
            if hasattr(self.args, "use_cosine") and self.args.use_cosine
            else False
        )
        # flag to use pseudo prototype gradients instead of the noise gradients
        self.use_pseudo_prototype = (
            True
            if hasattr(self.args, "use_pseudo_prototype")
            and self.args.use_pseudo_prototype
            else False
        )
        ...
        self.no_gradients = (
            True
            if hasattr(self.args, "no_gradients") and self.args.no_gradients
            else False
        )
        # flag to simply use the norm of the gradients instead of knn indexing
        self.use_grad_norm = (
            True
            if hasattr(self.args, "use_grad_norm") and self.args.use_grad_norm
            else False
        )
        self.use_direct_grad = (
            True
            if hasattr(self.args, "use_direct_grad")
            and self.args.use_direct_grad
            else False
        )
        self.filter_ood = (
            True
            if hasattr(self.args, "filter_ood") and self.args.filter_ood
            else False
        )
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        if self.use_synthetic:
            if self.synthetic_name == "synthetic":
                self.noise_proto_compute.get_synthetic_ood(self.n_noise_samples)
            else:
                print(
                    "Synthetic OOD name not recognized {}".format(
                        self.synthetic_name
                    )
                )
    ...
    def compute_ncc_loss(self, img):
        ...
        original_vector_prototypes = (
            original_feature.reshape(bsz, 1, -1).detach() - prototypes
        ).reshape(bsz, n_prototypes + 1, -1)
    ...
    def _compute_grad_directly(self, batch, data_alone=False):
        ...
        loss_data = self.compute_ncc_loss(
            data,
        )
    ...
    def _compute_grad(self, batch, data_alone=False):
        ...
        with torch.enable_grad():
            ...
            for i in range(bsz):
                ...
                gradients_proto = (
                    gradients_aug[1].data[pseudo_preds[i]].view(1, -1)
                )
    ...
    def update_prototype(self, batch):
        ...
        def get_total_features(feats_cl, cl, proto_tensor):
            ...
            return (
                feats_cl_sum + self._count_features[cl] * proto_tensor[cl]
            ) / (self._count_features[cl] + n_features)

        data, target = batch
        _, features = self.network(data, return_feature_list=True)
        pre_features = (
            features[self.pre_layer_indx].detach().reshape(data.shape[0], -1)
        )
        # for the prototypes
