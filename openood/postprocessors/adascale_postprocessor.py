...
class AdaScalePostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        if not self.setup_flag:
            ...
            feature_shift_log = []
            net.eval()
            self.feature_dim = net.feature_size
            ...
            num_samples = (
                ...
                if hasattr(self.args, "num_samples")
                else total_samples
            )
            ...
            topk_feature_shift_log = torch.gather(
                self.feature_shift_log, 1, topk_indices
            )
            sum_log = topk_feature_perturbed.sum(
                dim=1
            ) + self.lmbda * topk_feature_shift_log.sum(dim=1)
    ...
    def get_percentile(self, feature, feature_perturbed, feature_shift):
        ...
        topk_norm = topk_feature_perturbed.sum(
            dim=1
        ) + self.lmbda * topk_feature_shift.sum(
            dim=1
        )  # Q^{\prime}
        percent = 1 - self.ecdf(topk_norm.cpu().numpy())
        percentile = self.min_percentile + percent * (
    ...
    def postprocess(self, net: nn.Module, data):
        ...
        output = net.forward_threshold(data, percentile)
